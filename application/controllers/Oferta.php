<?php
	...
	*
	*/
	class Oferta extends REST_Controller
	{
...
//se valida si el resultado no es null de la respuesta
      ...
      header('Access-Control-Allow-Origin: *');
			if ($seleccion == 0) {
				...
				$this->response( array('oferta'=>$data), 200);
			}
...
//si el return de la funcion es true se imprime el resultado
	...
	public function index_post(){
		...
		$id = $this->oferta_model->save('data');

		if(!is_null($id)){
			$this->respose(array('oferta'=>$id),200);
		}
		else{
    ...
    public function index_put($id){
        ...
        $update = $this->oferta_model->update($id, $this->post('data'));

        if(!is_null($update)){
            $this->response(array('oferta'=>'correct update'),200);
        }
        else{
            $this->response(array('error'=>'no save'), 400);
        }
    ...
    public function index_delete($id){
        ...
        $delete = $this->oferta_model->delete($id);

        if(!is_null($delete)){
            $this->response(array('oferta'=>'correct delete'), 200);
        }
        else{
            $this->response(array('error'=>'no save'), 400);
        }
